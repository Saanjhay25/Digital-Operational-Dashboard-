...
export class IncidentService {
    ...
    static async assignIncident(incidentId: string, operatorId: string): Promise<void> {
        const response = await fetchApi(`/incidents/${incidentId}/assign`, {
            method: 'POST',
    ...
    static async getOperators(): Promise<any[]> {
        const response = await fetchApi('/users/operators');
        if (!response.ok) {
