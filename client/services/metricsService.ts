...
export class MetricsService {
    ...
    static async getErrorRate(): Promise<{ success: boolean; data?: { totalRequests: number; failedRequests: number; errorRate: number }; error?: string }> {
        try {
            const response = await fetchApi('/metrics/error-rate');

            if (!response.ok) {
                const data = await response.json();
                return { success: false, error: data.message || 'Failed to fetch error rate' };
            }

            const data = await response.json();
            return { success: true, data };
        } catch (e: any) {
            console.error('MetricsService.getErrorRate error:', e);
            return { success: false, error: 'Server unreachable' };
        }
    }
